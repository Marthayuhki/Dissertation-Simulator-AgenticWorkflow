...
| Safety Hook | GroundedClaim 검증 + Hallucination Firewall + query_step.py 결정론적 라우팅 + Step Consolidation (17 invocations / 211 steps) + Academic Search Pre-fetch (run_academic_search.py --auto-from-sot) |
...
L0: Anti-Skip Guard + Step Output Verification (verify_step_output.py)
  ...
  └── VO-8: 검색 프리페치 에이전트(SEARCH_PREFETCH_AGENTS)가 search-cache 파일을 참조하는지 사후 검증 (WARNING only)
...
### 4.2 인프라 스크립트 (67개 프로덕션 + 2개 공유 모듈)
...
| `query_step.py` | **Step Execution Registry** — 211-step 결정론적 agent/tier/critic/pCCS 매핑 + Step Consolidation/Execution P1 함수 4개 (`generate_consolidated_prompt`, `generate_single_step_prompt`, `get_next_execution_step`, `get_invocation_plan`) + `pre_execution_command` (검색 프리페치 에이전트용 P1 bash 명령) |
| `fallback_controller.py` | 3-tier Fallback 제어 (Team→Sub-agent→Direct) + `split_consolidated_group()` P1 결정론적 그룹 분할 |

**Hallucination Containment** (3):

| 스크립트 | 역할 |
|---------|------|
| `verify_step_output.py` | **Step Output Verification** — VO-1~VO-8 결정론적 산출물 검증. V-1(파일 경로), GAP-3(prefix 불일치), GAP-4(placeholder), GAP-DW(비박사급 글쓰기) 제거. VO-6(금지 표현 WARNING), VO-7(제목 구조 FAIL), VO-8(검색 캐시 참조 WARNING) |
| `determine_dialogue_outcome.py` | **Dialogue Outcome Decision** — All PASS→consensus, Any FAIL→continue/escalate. V-2(대화 루프 종료) 제거 |
| `run_academic_search.py` | **Academic Search Pre-fetch** — `--auto-from-sot` P1 결정론적 쿼리 추출 (SOT `research_question` 기반). 검색 결과를 `search-cache/step-N-results.json`에 캐시. `sot_registration_command` 출력으로 Orchestrator JSON 파싱 할루시네이션 제거 (ADR-075) |
...
### 4.3 Academic Search Pre-fetch Architecture (ADR-075)

Orchestrator Pre-fetch Pattern으로 학술 검색의 할루시네이션을 원천봉쇄한다:

```
Orchestrator (has Bash)          Agent (Read/Write/Glob/Grep/WebSearch only)
    │                                    │
    ├─ 1. query_step.py --step N         │
    │   → pre_execution_command 획득      │
    │                                    │
    ├─ 2. run_academic_search.py         │
    │   --auto-from-sot                  │
    │   → SOT research_question에서       │
    │     P1 결정론적 쿼리 추출            │
    │   → search-cache/step-N-results.json│
    │   → sot_registration_command 출력    │
    │                                    │
    ├─ 3. sot_registration_command 실행   │
    │   (checklist_manager.py             │
    │    --register-search-cache)          │
    │   → SOT outputs["step-N-search"]    │
    │     에 dict 메타데이터 기록           │
    │                                    │
    └─ 4. Agent 디스패치 ─────────────────▶ Read search-cache/step-N-results.json
                                          (LLM 쿼리 구성 0%, JSON 파싱 0%)
```

**4개 검색 프리페치 에이전트** (`SEARCH_PREFETCH_AGENTS`):
- `literature-searcher`, `seminal-works-analyst`, `trend-analyst`, `empirical-evidence-analyst`

**할루시네이션 벡터 봉쇄**:

| 벡터 | 위험 | P1 봉쇄 |
|------|------|---------|
| V-1 (쿼리 구성) | LLM이 검색 쿼리를 할루시네이션으로 구성 | `--auto-from-sot`: SOT `research_question`에서 결정론적 추출 |
| V-2 (프리페치 실행) | Orchestrator가 잘못된 프리페치 명령 구성 | `pre_execution_command`: `query_step.py`가 완전한 bash 명령 생성 |
| V-3 (SOT 등록) | Orchestrator가 JSON 필드를 잘못 파싱하여 등록 | `sot_registration_command`: `run_academic_search.py`가 완전한 bash 명령 출력 |
| V-4 (캐시 참조 검증) | 에이전트가 캐시를 무시하고 할루시네이션 데이터 사용 | VO-8: `verify_step_output.py`가 사후 참조 패턴 검증 (WARNING) |
| H-1 (캐시 무결성) | SOT에 등록됐지만 파일 삭제됨 | `is_search_cache_registered()`: SOT 엔트리 + 디스크 파일 존재 이중 검증 |
| H-2 (JSON 파싱) | Orchestrator가 5개 필드를 추출해 명령 조립 | `sot_registration_command`: 완전 조립된 명령으로 LLM 파싱 제거 |

**검색 캐시 SOT 구조** (`session.json.outputs`):

```json
{
  "step-39-search": {
    "cache_path": "search-cache/step-39-results.json",
    "total_results": 47,
    "databases_searched": ["crossref", "semantic_scholar"],
    "query": "Education AI safety alignment",
    "query_source": "sot"
  }
}
```

### 4.4 Context Memory 품질 최적화
...
| **Search Cache State** | `_surface_search_cache_state()`로 `search-cache/` 디렉터리의 캐시 파일 현황을 IMMORTAL 섹션에 표면화 → 프리페치 누락 방지 | `restore_context.py` |
...
| Safety Hook 체계 | GroundedClaim 검증 + Hallucination Firewall + `query_step.py` 결정론적 라우팅 + Academic Search Pre-fetch | `.claude/hooks/scripts/` |
...
| **Hallucination Containment** | V-1~V-4 취약점과 GAP-1~GAP-6+GAP-DW 커버리지 갭 + Academic Search V-1~V-4/H-1~H-2를 P1 결정론적 스크립트로 봉쇄하는 체계. verify_step_output.py(VO-1~VO-8) + determine_dialogue_outcome.py + split_consolidated_group() + H-8 단일 Step 프롬프트 P1 생성 + run_academic_search.py(--auto-from-sot) |
| **verify_step_output.py** | P1 결정론적 Step 산출물 검증. VO-1(파일 존재/크기) → VO-2(UTF-8) → VO-3(placeholder 미검출) → VO-4(GroundedClaim 존재) → VO-5(prefix 일치) → VO-6(금지 학술 표현 WARNING) → VO-7(제목 구조 FAIL) → VO-8(검색 캐시 참조 WARNING). V-1, GAP-3, GAP-4, GAP-DW 제거 |
...
| **Orchestrator Pre-fetch Pattern** | Orchestrator(Bash 가능)가 P1 스크립트로 데이터를 사전 수집 → 캐시 파일에 저장 → 에이전트(Bash 불가)가 Read로 참조하는 아키텍처. LLM 검색 쿼리/API 호출 할루시네이션 제거 |
| **run_academic_search.py** | P1 Academic Search Pre-fetch 스크립트. `--auto-from-sot` 모드로 SOT의 `research_question`에서 결정론적 쿼리 추출. `sot_registration_command` 출력으로 Orchestrator JSON 파싱 제거 |
| **SEARCH_PREFETCH_AGENTS** | 학술 검색 캐시의 혜택을 받는 4개 에이전트 집합: `literature-searcher`, `seminal-works-analyst`, `trend-analyst`, `empirical-evidence-analyst` |
| **pre_execution_command** | `query_step.py`가 반환하는 P1 결정론적 bash 명령. Orchestrator가 에이전트 디스패치 전에 실행 (검색 프리페치용) |
| **sot_registration_command** | `run_academic_search.py`가 출력하는 P1 결정론적 bash 명령. 검색 캐시를 SOT에 등록 (`--register-search-cache`). Orchestrator JSON 파싱 할루시네이션 제거 |
| **search-cache/** | 학술 검색 결과 캐시 디렉터리. `step-N-results.json` 형식. `run_academic_search.py`가 생성, 에이전트가 Read로 참조 |
